...
import { Modal, ModalContent, ModalHeader, ModalBody, useDisclosure, Avatar, Link, ModalFooter, Button } from "@nextui-org/react";
...
const Avatars = ({ onSave, avatar }: IProps) => {
	...
	const [selected, setSelected] = useState<string | null | undefined>(avatar?.split('/')[3]);
	...
	return (
		<>
			...
			<div className='flex flex-col items-center justify-center relative'>
				<Avatar src={`${selected ? `/img/avatars/${selected}` : ''}`} size='lg' />
				<div onClick={onOpen} className='absolute -bottom-1 -right-2 z-50 w-6 h-6 rounded-full bg-white dark:bg-default dark:border-2 dark:border-background flex items-center justify-center cursor-pointer hover:text-primary'>
				...
				<ModalContent>
					{(onClose) => (
						<>
							...
							<ModalHeader className="flex flex-col gap-1 text-md font-bold">Avatars</ModalHeader>
							...
							<ModalFooter>
								...
								<Button color="primary" isDisabled={selected === null} onPress={onClose}>
									Guardar
								</Button>
							</ModalFooter>
