...
import { Avatar, Button, Checkbox, CheckboxGroup, Input, Link, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Radio, RadioGroup, useDisclosure } from '@nextui-org/react'
...
import { setPlanningId, setProfile, setTeamPoints } from '../../infrastructure/redux'
...
const SessionInvitado = ({ titlePlanning, descriptionPlanning, planningId, teams, protectPlanning }: IProps) => {
	const { isOpen, onOpen, onOpenChange } = useDisclosure();
	const [passwordSucess, setPasswordSucess] = useState(false);
	const dispatch = useDispatch();
	const router = useRouter();

	const { handleSubmit, control, trigger, formState: { errors } } = useForm<FormDataAccesoPlanning>({
	...
	const { createGuestTemporal, loading } = servicesPlanningLive.createGuestTemporal(onSuccess);

	const { modalValidatePassword } = servicesPlanningLive.validatePasswordPlanning((result: any) => {
	...
	console.log('session de invitados.')
		...
		<>
			<div className='w-screen h-screen flex flex-col items-center justify-center'>
				...
				<div className='w-[400px] flex flex-col items-center bg-white rounded-lg p-8 shadow-card'>
					...
					<div className='relative w-56 h-10'>
						<Image src='/img/logoColor.svg' alt='Logo Planning poker life' layout="fill" objectFit="contain" />
					</div>

					<span className='text-sm mt-2 block'>{titlePlanning}</span>
					<small className='text-xs text-secondary-300'>{descriptionPlanning}</small>

					{ErrorPlanning && <span className='font-bold text-sm my-2 text-red-600'>{ErrorPlanning}</span>}
					...
					<div className='w-full space-y-4 my-4'>
						...
						<div className=' bg-secondary-100/50 rounded-md p-2'>
							...
							>
								{teams.length > 0 && teams.map((item: ITeam) => <Radio classNames={{ label: 'text-sm' }} key={item._id} value={item._id}>{item.name}</Radio>)}
						...
						<Controller
							...
							render={({ field: { onChange, value } }) => (
								<Checkbox checked={value} onChange={onChange} size='sm'>Entrar en modo Observador</Checkbox>
							)}
						/>
						<Button type="button" isLoading={loading} onClick={handleSubmit(onSubmit)} color="primary" size='md' className='w-full text-md' radius='full'> Entrar a la planning</Button>
			...
			<Modal isOpen={isOpen} onOpenChange={onOpenChange}>
				<ModalContent>
					{(onClose) => (
						<>
							<ModalHeader className="flex flex-col gap-1">Planning protegida</ModalHeader>
							<ModalBody>
								<p className=' text-xs text-secondary-400'>Ingrese la contraseña de la planning para acceder</p>
								<Input
									size='md'
									radius='sm'
									type='password'
