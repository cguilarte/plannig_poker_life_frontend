...
interface IProps {
	data: [] | IIssues[];
	setData: (issues: [] | IIssues[] | any[]) => void;
	idList: String;
}
...
const CardList = ({ data, setData, idList }: IProps) => {
	...
	/* 	const onDelete = (item: IIssues) => {
			setData(data.filter((row: IIssues) => row.key !== item.key));
		}
	 */
				...
				<Droppable isDropDisabled={!isControl} droppableId={`droppables_task_${idList}`}>
					{(provided: any) => (
						<div {...provided.droppableProps} ref={provided.innerRef}
							className='flex flex-col space-y-4 pb-2 h-[72vh] overflow-y-scroll'>
							{data.map((item: IIssues, index) => (
