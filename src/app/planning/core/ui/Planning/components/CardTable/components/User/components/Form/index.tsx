...
const Form = ({ isOpen, onOpenChange, onClose }: any) => {
  ...
  const { updatePerfilGuest, loading } =
    servicesPlanningLive.updatePerfilGuest(onSuccess);
  ...
  const {
    ...
    setError,
    formState: { errors },
  } = useForm<FormDataUpdatePerfilGuest>({
                    ...
                    >
                      {teams.length > 0 &&
                        teams.map((item: ITeam) => (
                          <Radio
                            classNames={{ label: "text-sm dark:text-white/50" }}
                            key={item._id}
                            value={item._id}
                          >
                            {item.name}
