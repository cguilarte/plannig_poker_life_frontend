      ...
      >
        <PopoverTrigger>
          <Button
            size="sm"
            color="primary"
            className="bg-white text-sm dark:bg-default dark:text-white"
            variant="flat"
            startContent={<TbUserPlus className="text-lg dark:text-white" />}
          >
            Invitar
          </Button>
        </PopoverTrigger>
        <PopoverContent>
          <div className=" flex flex-col px-1 py-2">
            ...
            <p className="text-xs text-secondary-400 dark:text-white/80">
              ¡Comparte este enlace o código QR con tu equipo para comenzar!
            </p>
            ...
            <div className="flex flex-row items-center space-x-4 mt-4 ">
              <div className="flex flex-col space-y-3">
                <Button
                  ...
                  onClick={() =>
                    copyLink(
                      `${process.env.NEXT_PUBLIC_URL_FRONT}/${ROUTER.planningLiveUrl}/${info.planningId}`
                    ).then(() => alertSuccess("ID copiado con éxito."))
                  }
                  variant="solid"
              ...
              <div
                ref={qrRef}
                className="p-2 border-solid border-1 border-secondary-200 rounded-lg flex items-center justify-center"
              >
                <QRCodeCanvas
                  id="qrCode"
                  size={60}
                  value={`${process.env.NEXT_PUBLIC_URL_FRONT}/${ROUTER.planningLiveUrl}/${info.planningId}`}
                />
                ,
              </div>
            </div>
