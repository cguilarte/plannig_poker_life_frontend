      ...
      >
        <PopoverTrigger>
          <Button
            size="sm"
            color="primary"
            className="text-sm dark:hover:bg-default"
            variant="light"
            isIconOnly
            radius="full"
            startContent={
              <Image
                src="/img/emoji/good.svg"
                width={30}
                height={30}
                alt="Emoji"
              />
        ...
        <PopoverContent className="dark:bg-[#3C4043]">
          <div className="w-auto flex items-center justify-center">
            {EMOJI_MAP.map((row: string, index: number) => (
              <button
                onClick={() => handleSendEmoji(row)}
                key={index}
                className="w-10 h-10 transition-all rounded-full text-2xl hover:bg-primary-200 dark:hover:bg-black/50"
              >
                {row}
              </button>
            ))}
