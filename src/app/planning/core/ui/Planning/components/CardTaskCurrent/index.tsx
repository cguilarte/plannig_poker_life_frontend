...
const CardTaskCurrent = ({ taskCurrent }: IIssues | ITask | any) => {
  ...
  return (
    <>
      <div
        className={`w-full flex items-center justify-center pt-0 transition-all hover:scale-105 cursor-pointer ${
      ...
      >
        <div className=" rounded-lg bg-white dark:bg-default py-1 px-2 flex items-center text-xs space-x-2 dark:space-x-0 shadow-md">
          <Chip
            color="primary"
            size="sm"
            className="mr-2 font-bold text-xs"
            variant={theme === "dark" ? "light" : "flat"}
            startContent={<FiExternalLink />}
          >
            {task?.key}
          </Chip>
          <span onClick={onOpen} className="hover:underline pr-4 block">
            {task?.title}
          </span>
          <IoIosRemoveCircleOutline
            onClick={handleRemoveTask}
            className=" text-red-700 dark:text-white text-lg  transition-all hover:scale-125 cursor-pointer"
          />
        </div>
      </div>
