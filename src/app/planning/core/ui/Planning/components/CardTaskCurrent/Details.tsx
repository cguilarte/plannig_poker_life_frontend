				...
				{() => (
					<>
						<ModalHeader className="flex flex-col">
							...
							{/* 							{item.project && <div className='flex items-center text-sm text-secondary-300'>Proyecto <span className='mx-2'>/</span> <img className='mr-1' src={item?.project.avatarUrls} width={12} height={12} alt='logo' /> {item?.project.name}  <span className='mx-2'>/</span> {item.key}</div> }
 */}						</ModalHeader>
