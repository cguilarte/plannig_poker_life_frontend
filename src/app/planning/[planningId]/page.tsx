...
import {
  ...
  setVoto,
  setVotoNew,
} from "../core/infrastructure/redux";
import { useApolloClient, useSubscription } from "@apollo/client";
import {
  GET_PLANNING_DETAIL_LIVE,
  SUSCRIPTION_ACTIVATE_RESTORE_DATA,
  SUSCRIPTION_ADD_VOTO,
  SUSCRIPTION_CONTROL_CELEBRATION_PLANNING,
  SUSCRIPTION_CONTROL_PLANNING,
  SUSCRIPTION_DELETE_TASK,
  SUSCRIPTION_DESCONECTED_GUEST,
  SUSCRIPTION_DONE_PLANNING,
  SUSCRIPTION_LIST_GUEST,
  SUSCRIPTION_NEW_VOTO,
  SUSCRIPTION_REMOVE_GUEST,
  SUSCRIPTION_RESTORE_DATA,
  SUSCRIPTION_RESULT_PLANNING,
  SUSCRIPTION_SELECT_TASK,
  SUSCRIPTION_SHOW_CARD,
  SUSCRIPTION_UPATED_PERFIL_GUEST,
  SUSCRIPTION_UPDATE_PLANNING,
  SUSCRIPTION_UPDATE_TASK,
} from "../core/infrastructure/graphql";
...
export default function Page() {
  ...
  const { loading, data } = servicesPlanningLive.detailPlanningId(
    param.planningId as string
  );
      ...
      if (data.data) {
        ...
        guests.forEach((guest: any) => {
          if (teamsCurrent.length > 0) {
            const findTeam = teamsCurrent.find(
              (item: any) => item.id === guest?.team?._id
            );
            if (!findTeam) {
              teamsCurrent.push({
                id: guest.team._id,
    ...
    onData: () => {
      ...
      if (result) {
        ...
        if (resultData.estimatedValues) {
          resultData.estimatedValues = resultData.estimatedValues.map(
            (row: any) => {
              return {
                key: row.key,
                value: row.value,
  ...
  useSubscription(SUSCRIPTION_SELECT_TASK, {
    ...
    onData: ({ data }: any) => {
      if (data.data) {
        ...
        let task = {
          ...infoPlanning.task.find((item: any) => item._id === taskId),
        };
        if (subtaskId) {
      ...
      if (data.data) {
        ...
        if (votos && votos.length > 0) {
          votos.forEach((row: any) => {
            const findTeamIndex = listTeamPoints.findIndex(
              (item: any) => item.id === row.teamId
            );
            if (findTeamIndex !== -1) {
              listTeamPoints[findTeamIndex] = {
                ...listTeamPoints[findTeamIndex],
                point: listTeamPoints[findTeamIndex].point + row.voto,
              };
            }
          });
        ...
        if (newVoto) {
          const indexTask = cloneInfoPlanning.task.findIndex(
            (item: any) => item._id === taskId
          );
          if (indexTask !== -1) {
  ...
  useSubscription(SUSCRIPTION_DELETE_TASK, {
    ...
    onData: ({ data }: any) => {
      if (data.data) {
        ...
        const task = cloneInfoPlanning.task.filter(
          (item: any) => item._id.toString() !== idTask.toString()
        );
  ...
  if (!profile || profile.planningId !== param.planningId)
    return (
      <SessionInvitado
        protectPlanning={data.protectPlanning}
        titlePlanning={data.title}
        descriptionPlanning={data.description}
        planningId={param.planningId as string}
        teams={data.teams}
      />
    );
